<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
                <div class="p-5 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                    <h3 class="font-semibold text-gray-800 dark:text-gray-100">Daftar Alat Belum Dikembalikan</h3>
                    <span class="text-xs text-gray-400 dark:text-gray-500">Total: {{ $peminjaman->count() }} Pengembalian</span>
                </div>
                    ...
                    <table class="w-full text-sm text-left">
                        <thead>
                            <tr class="border-b border-gray-200 dark:border-gray-700">
                                ...
                                <th class="pb-3 pr-4 text-gray-600 dark:text-gray-300 font-semibold">Kondisi Awal</th>
                        ...
                        <tbody id="tableBody" class="divide-y divide-gray-100 dark:divide-gray-700">
                            @forelse($peminjaman as $i => $p)
                                <tr data-row class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                    ...
                                    <td class="py-3">
                                        <button
                                            onclick="openProsesModal(
                                                {{ $p->id }},
                                                '{{ addslashes($p->alat->nama_alat) }}',
    ...
    <div id="modalProses" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black/50">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md mx-4">
            <div class="p-5 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                <h3 class="font-semibold text-gray-800 dark:text-gray-100">Proses Pengembalian</h3>
                <button onclick="closeModal('modalProses')"
                    class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-xl leading-none">&times;</button>
            </div>
            <div class="p-5">
                {{-- Info ringkas --}}
                <div class="bg-gray-50 dark:bg-gray-700 rounded p-3 mb-4 space-y-1 text-sm">
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Nama Alat</span>
                        <span id="infoNamaAlat" class="font-medium text-gray-800 dark:text-gray-100"></span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Peminjam</span>
                        <span id="infoPeminjam" class="font-medium text-gray-800 dark:text-gray-100"></span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Kondisi Awal</span>
                        <span id="infoKondisiAwal" class="font-medium text-gray-800 dark:text-gray-100"></span>
                    </div>
                </div>

                <form id="formProses" action="" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')
    ...
    <script>
        ...
        function openProsesModal(id, namaAlat, namaPeminjam, kondisiAwal) {
            document.getElementById('infoNamaAlat').textContent = namaAlat;
            document.getElementById('infoPeminjam').textContent = namaPeminjam;
            document.getElementById('infoKondisiAwal').textContent = kondisiAwal === 'baik' ? 'Baik' : 'Rusak';
            document.getElementById('formProses').action = '/pengembalian/' + id;
